...
export default function Chat() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (sid) {
      ...
      newPoints = refreshed.puntos;
      newLevelName = refreshed.nivel_actual || getLevelInfo(newPoints).currentLevelName;
    } else {
      newPoints = (user.points || 0) + 1;
      const info = getLevelInfo(newPoints);
      newLevelName = info.currentLevelName;
    }

    if (oldLevelName !== newLevelName) {
      updateUser({ ...user, points: newPoints, nivel_actual: newLevelName } as any);
